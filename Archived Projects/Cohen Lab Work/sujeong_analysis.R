...
setwd("/home/atom/Desktop/Data/sujeong")

#Loads in count matrices and metadata. Appropriately converts them to factors and aligns
# column names.
mdp_counts <- data.frame(read_excel("NormalizedReadCount_SENvsNON.xlsx"))
rownames(mdp_counts) <- mdp_counts[,1]
mdp_counts <- mdp_counts[,-1]
mitogene_counts <- data.frame(read_excel("Largegenechange_SEN.xlsx"))
rownames(mitogene_counts) <- mitogene_counts$Feature
mitogene_counts <- mitogene_counts[,-1]
sujeong_samplesheet <- read.csv("sujeong_samplesheet.csv")
colnames(mdp_counts) <- sujeong_samplesheet$Sample
colnames(mitogene_counts) <- sujeong_samplesheet$Sample
mitogene_rownames <- rownames(mitogene_counts)
mitogene_counts <- data.frame(lapply(mitogene_counts,as.integer))
rownames(mitogene_counts) <- mitogene_rownames
mdp_rownames <- rownames(mdp_counts)
mdp_counts <- data.frame(lapply(mdp_counts,as.integer))
rownames(mdp_counts) <- mdp_rownames
...
mdp_counts <- mdp_counts[1:464,]
remove_rows <- c("169B","167B","175B","39A","61A","40A","59A")
mdp_counts <- mdp_counts[!(rownames(mdp_counts) %in% remove_rows),]
...
ggplot(data=mitogene_res, aes(x=log2FoldChange, y=-log10(padj), color=color, label=delabel)) + 
  ...
  geom_vline(xintercept = -.6,linetype="dotted") +
  labs(title="MitoGene Volcano Plot for Senescence")

mitogene_gtf <- read_excel("~/Desktop/Data/mtDNAlocation.xlsx")
mdp_gtf <-
...
encompass_table <- generateEncompassTable(mdp_gtf,mitogene_gtf,TRUE)
encompass_table <- encompass_table[encompass_table$mitogene %in% mitogene_rownames,]
...
mdp_res_corrected$condition <- "Senescence"
write.csv(mdp_res_corrected,"SenescenceMDPSeq.csv")
mdp_vsd_corrected <- varianceStabilizingTransformation(mdp_dds_corrected) ###mdp-seq script
mdp_resOrdered_corrected <- mdp_res_corrected[order(abs(mdp_res_corrected$padj),decreasing=FALSE),]
...
ggplot(data=mdp_res_corrected, aes(x=log2FoldChange, y=-log10(padj), color=color, label=delabel)) + 
  ...
  geom_text(nudge_x=.2) +
  scale_colour_identity() +
  labs(title="MDPSeq Volcano Plot for Senescence - Corrected")
...
ggplot(data=mdp_res_uncorrected, aes(x=log2FoldChange, y=-log10(padj), color=color, label=delabel)) + 
  ...
  geom_text(nudge_x=.2) +
  scale_colour_identity() +
  labs(title="MDPSeq Volcano Plot for Senescence - Uncorrected")
