...
generateEncompassTable <- function(gtf_mdp, gtf_mitochondrial, stranded=FALSE) {
  ...
  mdp_db <- drop_na(mdp_db[-c(62, 425), ])
  if (!stranded) {
  ...
  if (stranded) {
    #Custom analysis for stranded count matrix makes me do MDP pre-processing in here.
    mitogene_db <- data.frame(gtf_mitochondrial)
    colnames(mitogene_db) <- c("mitogene_name","start","end","strand")
  }
  ...
  for (mdp_row in 1:nrow(mdp_db)) { # Loops through all MDPs.
    ...
    mdp_sense <- mdp_db[mdp_row,"sense"]
    if (!stranded) {
    ...
    if (stranded) {
      for (mito_row in 1:nrow(mitogene_db)) { # Loops through all mitogenes.
        mito_s <- mitogene_db[mito_row, "start"]
        mito_e <- mitogene_db[mito_row, "end"]
        mito_sense <- mitogene_db[mito_row,"strand"]
        mito_size <- mito_e-mito_s # Mitogene size useful for calculating % overlap.
        if ((mdp_s < mito_s & # No overlap.
             mdp_s < mito_e & mdp_e < mito_s & mdp_e < mito_e) |
            (mdp_s > mito_s &
